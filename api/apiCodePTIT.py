...
class ApiCodePTIT:
    ...
    def getCookie(self):
        ...
        login = requests.post(url=f'{self.host}/login',
                              data=data, headers=self.header)
        self.header.update({'Cookie': login.headers.get(
            'Set-Cookie').split('samesite=lax, ')[1]})

        try:
            self.name = login.text.split('<p class="nav__profile__menu__name">')[
                1].split('</p>')[0]
            self.id = login.text.split('<p class="nav__profile__menu__code">')[
                1].split('</p>')[0]
        except IndexError:
            print(
    ...
    def getGroupList(self):
        groups = requests.get(url=f'{self.host}/student/question',
                              headers=self.header)
    ...
    def getQuestionsInPage(self, page):
        ...
        for problem in problems:
            ...
            if 'bg--10th' in problem.get('class', []):
                ...
                completeQuestionsList.append(ProblemInfo(
                    problemName, problemLink, problemID, problemStatus, problemGroup, problemTopic, problemDifficulty))
            else:
                problemStatus = 'Incomplete'
                incompleteQuestionsList.append(ProblemInfo(
                    problemName, problemLink, problemID, problemStatus, problemGroup, problemTopic, problemDifficulty))
    ...
    def getSolutionQuestion(self, question=ProblemInfo):
        questions = requests.get(
            url=f'{question.problemLink}', headers=self.header)
        solutionList = []
        index = 0
        while True:
            try:
                solutionTable = BeautifulSoup(
                    questions.text, 'html.parser').find_all('tbody')[index]

                solutionTable = solutionTable.find_all('tr')
                for solution in solutionTable:
                    solutionID = solution.find_all('td')[0].text.strip()
                    solutionTimeSubmit = solution.find_all('td')[
                        1].text.strip()
                    ...
                    solutionLink = solution.find_all(
                    ...
                    solutionList.append(SolutionInfo(solutionID, solutionTimeSubmit, solutionName,
                                                     solutionLink, solutionStatus, solutionTime, solutionLanguage, solutionMemory))
                break
            except IndexError:
                index += 1
        return solutionList
